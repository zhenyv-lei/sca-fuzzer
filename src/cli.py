...
def main() -> int:
    ...
    # ==============================================================================================
    # Mutation for discarded test cases
    parser_tfuzz = subparsers.add_parser('mutate', add_help=True, parents=[common_parser])
    parser_tfuzz.add_argument(
        "-n",
        "--num-test-cases",
        type=int,
    ...
    parser_tfuzz.add_argument(
        ...
        type=str,
        required=True,
        help="The template to use for generating test cases")#暂且使用这个参数来传入需要变异的测试用例
    parser_tfuzz.add_argument(
        '--timeout',
        type=int,
        default=0,
    ...
    # Mutation for discarded test cases
    if args.subparser_name == 'mutate':
        testcase = args.template
        fuzzer = get_fuzzer(args.instruction_set, args.working_directory, testcase, "")
        exit_code = fuzzer.start_for_mutate(args.num_test_cases, args.num_inputs,
                                                   args.timeout, args.nonstop, args.save_violations)
        return exit_code
