...
class FuzzerGeneric(Fuzzer):
    ...
    def initialize_modules(self):
        ...
        #print("\n============ fuzzer.py  get_program_generator()  come soon  ==========================")
        self.generator = factory.get_program_generator(isa, prog_seed)#这一行会调用 X86Generator  init 以及 RandomGenerator  init
        #使用的是这个类 class X86RandomGenerator(X86Generator, RandomGenerator)
        #print("\n============ fuzzer.py  get_program_generator()  end  ==========================")
    ...
    def start_random(self, num_test_cases: int, num_inputs: int, timeout: int, nonstop: bool,
        ...
        self.generation_function = self.generator.create_test_case  #这里只是赋值操作，不会执行create函数
    ...
    def analyse_traces_from_files(ctrace_file: str, htrace_file: str):
        ...
        # print results
        if violations:
            print("\n\n================================ fuzzer.py  analyse_traces_from_files()==========================")
