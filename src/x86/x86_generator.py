...
class X86Printer(Printer):
    ...
    def print(self, test_case: TestCase, outfile: str, print_exit: bool = True) -> None:
        with open(outfile, "w") as f:
            ...
            # print the test case
            for func in test_case.functions:
                self.print_function(func, f, print_exit)

            # print epilogue
            for line in self.epilogue_template:
                f.write(line)

    def print_function(self, func: Function, file, print_exit: bool = True):
        file.write(f".section .data.{func.owner.name}\n")
        file.write(f"{func.name}:\n")
        #counter = 0
        for bb in func:
            #file.write(f"# Basic Block {counter} start\n")
            self.print_basic_block(bb, file)
            #file.write(f"# Basic Block {counter} end\n")
            #counter += 1

        #file.write(f"000000000000000\n")
        if print_exit:
            self.print_basic_block(func.exit, file)#tfuzz模式下，这行代码会在asm中多产生一个exit标签，导致报错；fuzz模式功能应该是正常的
        #file.write(f"11111111111111111\n")

    def print_basic_block(self, bb: BasicBlock, file):
        file.write(f"{bb.name.lower()}:\n")#tfuzz模式下额外的exit来自于这里
        for inst in bb:
...
class X86RandomGenerator(X86Generator, RandomGenerator):
    ...
    def mutate_for_discarded_test_case(self, discarded: str, enable_spec_mutate: bool = False) -> TestCase:
        """
        对被淘汰的用例做复活变异
        :param discarded: 原始汇编文件路径
        :param enable_spec_mutate: True -> 启用 speculation 触发变异
        :return: 变异后的测试用例
        """
        if self.parsed_template:
            test_case = deepcopy(self.parsed_template)
        else:
            test_case = self.asm_parser.parse_file(discarded)
        ...
        for func in test_case.functions:
            for bb in func:
                for instr in bb:
                    ...
                    #这个模式下暂且复用这一条属性，代表来自discarded用例本身

        if enable_spec_mutate:
            self.mutate_speculation_s1(test_case)


        for p in self.passes:
            p.run_on_test_case(self.test_case)
        
        asm_file = 'mutated.asm'
    ...
    def mutate_speculation_s1(self, tc: TestCase) -> None:
        """
        :param tc: 目标测试用例
        :return: None
        """
        func_0 = tc.functions[0]
        bb_0 = func_0[0]
        if len(bb_0.terminators) != 2:           # 必须是双后继
            return
    
        term_0 = bb_0.terminators[0]
        label = term_0.get_label_operand()#拿到要跳转的基本块名字代表的操作数对象
        target_bb_name = label.value
        # 通过名字反查 BasicBlock 对象
        target_bb = None
        for bb in func_0:
            if bb.name == target_bb_name:
                target_bb = bb
                break
        ...
        #插入push rbx
        pos = term_0.previous if term_0.previous else bb_0.start
        push_inst = self.push_rbx()
        bb_0.insert_after(pos, push_inst)
        pos = push_inst

        #插入and rcx, 63（掩码防 #PF）
        and_inst = Instruction("and", False, "BASE-LOGICAL", False,
                           [RegisterOperand("rcx", 64, src=True, dest=True),
                            ImmediateOperand("63", 64)])
        bb_0.insert_after(pos, and_inst)
        pos = and_inst

        #cmp rcx, 64
        cmp_inst = Instruction("cmp", False, "BASE-BINARY", False,
                           [RegisterOperand("rcx", 64, src=True, dest=False),
                            ImmediateOperand("64", 64)])
        bb_0.insert_after(pos, cmp_inst)
        pos = cmp_inst

        #越界load
        load_inst = Instruction("mov", False, "BASE-DATAXFER", False,
                            [RegisterOperand("rax", 64, dest=True, src=False),
                             MemoryOperand("[r14 + rcx*8]", 64, src=True, dest=False)])
        bb_0.insert_after(pos, load_inst)
        pos = target_bb.start

        #后继基本块插入越界store
        store_inst = Instruction("mov", False, "BASE-DATAXFER", False,
                             [MemoryOperand("[r14 + rax]", 64, dest=True, src=False),
                              RegisterOperand("rbx", 64, src=True, dest=False)])
        target_bb.insert_before(pos, store_inst)
        pos = store_inst

        #插入pop rbx
        pop_inst = Instruction("pop", False, "BASE-POP", False)
        pop_inst.add_op(RegisterOperand("rbx", 64, src=True, dest=False))
        # 隐式 rsp（先读后写）
        pop_inst.add_op(RegisterOperand("rsp", 64, src=True, dest=True), implicit=True)
        # 隐式 [rsp]（写入栈顶）
        pop_inst.add_op(MemoryOperand("[rsp]", 64, src=True, dest=False), implicit=True)
        target_bb.insert_before(pos, pop_inst)

    def push_rbx(self) -> Instruction:
        inst = Instruction("push", False, "BASE-PUSH", False)
        # 显式源操作数
        inst.add_op(RegisterOperand("rbx", 64, src=True, dest=False))
        # 隐式 rsp（先读后写）
        inst.add_op(RegisterOperand("rsp", 64, src=True, dest=True), implicit=True)
        # 隐式 [rsp]（写入栈顶）
        inst.add_op(MemoryOperand("[rsp]", 64, src=False, dest=True), implicit=True)
        return inst
