...
class X86Fuzzer(FuzzerGeneric):
    ...
    def _start(self, num_test_cases: int, num_inputs: int, timeout: int, nonstop: bool,
        ...
        check_instruction_list(self.instruction_set)#检查instruction_set是否包含用于产生特定fault的必要指令，并在缺失某些关键指令或出现已知有问题的指令时打印警告。
        print("\n============  X86Fuzzer() _start() =============\n")
        ...
        with quick_and_dirty_mode(self.executor):
            ...
            if CONF.enable_speculation_filter:
                for i, htrace in enumerate(org_htraces):
                    ...
                    if pfc_values[0] > pfc_values[1] or pfc_values[2] > 0:
                        print(f"Speculation filter: i={i}, htrace={htrace}, pfc_values={pfc_values}")
                        break
                    
                else:
                    STAT.spec_filter += 1
                    '''
                    # 创建用于保存未触发推测执行而被丢弃的测试用例的目录
                    timestamp = datetime.today().strftime('%y%m%d-%H%M%S')
                    speculation_dir = f"discarded/{timestamp}-specualtion"
                    Path(speculation_dir).mkdir(parents=True, exist_ok=True)

                    # 将被丢弃的测试用例及输入保存到指定目录中
                    test_case.save(f"{speculation_dir}/program.asm")
                    for i, input_ in enumerate(inputs):
                        input_.save(f"{speculation_dir}/input_{i:04}.bin")

                    # 保存配置文件
                    if CONF._config_path:
                        shutil.copy2(CONF._config_path, f"{speculation_dir}/org-config.yaml")
                    else:
                        with open(f"{speculation_dir}/org-config.yaml", "w") as f:
                            f.write("# Original violation used a default config, hence this file is empty")
                '''                         
                    # True means that the test case should be filtered out
                    return True
            ...
            if CONF.enable_observation_filter:
                ...
                if traces_match:
                    ...
                    #创建用于保存未触发推测执行而被丢弃的测试用例的目录
                    timestamp = datetime.today().strftime('%y%m%d-%H%M%S')
                    observation_dir = f"discarded/{timestamp}-observation"
                    Path(observation_dir).mkdir(parents=True, exist_ok=True)

                    # 将被丢弃的测试用例及输入保存到指定目录中
                    test_case.save(f"{observation_dir}/program.asm")
                    for i, input_ in enumerate(inputs):
                        input_.save(f"{observation_dir}/input_{i:04}.bin")

                     # 保存配置文件
                    if CONF._config_path:
                        shutil.copy2(CONF._config_path, f"{observation_dir}/org-config.yaml")
                    else:
                        with open(f"{observation_dir}/org-config.yaml", "w") as f:
                            f.write("# Original violation used a default config, hence this file is empty")
                    '''
                    # True means that the test case should be filtered out
                    return True
